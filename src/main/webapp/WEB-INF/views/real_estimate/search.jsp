...
<div class="search"><input type="text" name="search" placeholder="지역명, 원룸, 신축 "><button>검색</button></div>
	...
	<div class="search">
		<select name="type" onchange="formChange(this.form)">
			<option value="none" <c:if test="${type eq 'none'}">selected</c:if>>방 타입</option>
	...
	<div class="search">
		<select name="setype" onchange="formChange(this.form)">
			<option value="none" <c:if test="${setype eq 'none'}">selected</c:if>>세 종류</option>
	...
	<div class="menu">
		<select name="money" onchange="formChange(this.form)">
			...
			<option value="max" <c:if test="${money eq 'max'}">selected</c:if>>120만원 이상</option>
			</c:if>
			...
			<option value="max" <c:if test="${money eq 'max'}">selected</c:if>>3억 이상</option>
			</c:if>
			</select>
	</div>
	<div class="search">
	...
	<div class="search" style="float:right">
		<input type="button" value="매물 등록" onclick="location.href='enrollform.re'">
	</div>
